    ...
    <div class="card-body">
        <?php if (!empty($created_credentials)): ?>
            <div class="alert alert-success" style="margin-bottom: 1rem;">
                <strong>Dane logowania nowego uzytkownika</strong>
                <div style="margin-top: 0.5rem;">
                    <div><strong>Login:</strong> <span id="created-login"><?php echo e($created_credentials['username']); ?></span></div>
                    <div><strong>Email:</strong> <span id="created-email"><?php echo e($created_credentials['email']); ?></span></div>
                    <div><strong>Haslo:</strong> <span id="created-password"><?php echo e($created_credentials['password']); ?></span></div>
                </div>
                <div style="margin-top: 0.75rem;">
                    <button type="button" class="btn btn-sm btn-secondary" id="copy-created-credentials">Kopiuj dane logowania</button>
                </div>
            </div>
        <?php endif; ?>
                ...
                <div class="form-group col col-12 col-md-6">
                    ...
                    <select id="role_id" name="role_id" class="form-control" required>
                        ...
                        <?php foreach ($roles as $role): ?>
                            <option value="<?php echo (int)$role['id']; ?>" <?php echo (string)$form['role_id'] === (string)$role['id'] ? 'selected' : ''; ?>>
                                <?php echo e($role['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <?php if (!empty($errors['role_id'])): ?>
                        <div class="form-error"><?php echo e($errors['role_id']); ?></div>
                    <?php endif; ?>
                </div>
            </div>
...
(function() {
    ...
    const copyButton = document.getElementById('copy-created-credentials');
    if (copyButton) {
        copyButton.addEventListener('click', function() {
            const login = document.getElementById('created-login')?.textContent || '';
