...
class ManagementDocumentationController extends Controller {
    public function index() {
        requireLogin();

        $rbac = new RBAC();
        if (!$rbac->hasRole('Zarząd') && !$rbac->isAdmin()) {
            setFlashMessage('error', 'Brak dostepu do dokumentow Zarzadu KOT.');
            $this->redirectTo('/index.php');
        }

        $docs_dir = BASE_PATH . '/docs/management';
        if (!is_dir($docs_dir)) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
                ...
                $this->redirectTo('/management/documentation.php');
            }
            ...
            if ($action === 'create') {
                ...
                if ($file_name === '') {
                    ...
                    AuditLog::log('management_docs.create', 'management_docs', null, null, ['file' => $file_name]);
                    setFlashMessage('success', 'Dokument zostal dodany.');
                }

                $this->redirectTo('/management/documentation.php' . ($file_name !== '' ? '?file=' . urlencode($file_name) : ''));
            }
            ...
            if ($action === 'upload') {
                ...
                if ($error_code === UPLOAD_ERR_NO_FILE) {
                    ...
                    AuditLog::log('management_docs.upload', 'management_docs', null, null, ['file' => $file_name]);
            ...
            if ($action === 'delete') {
                ...
                if ($file_name === '' || $file_path === null) {
                    ...
                    AuditLog::log('management_docs.delete', 'management_docs', null, null, ['file' => $file_name]);
            ...
            if ($action === 'update') {
                ...
                if ($file_name === '' || !in_array(strtolower(pathinfo($file_name, PATHINFO_EXTENSION)), ['txt', 'md'], true)) {
                    ...
                    AuditLog::log('management_docs.update', 'management_docs', null, null, ['file' => $file_name]);
                    setFlashMessage('success', 'Dokument zostal zaktualizowany.');
                } else {
                    setFlashMessage('error', 'Blad podczas zapisywania dokumentu.');
                }

                $this->redirectTo('/management/documentation.php?file=' . urlencode($file_name));
            }
        }
        ...
        $this->render('management/documentation', [
            'page_title' => 'Dokumenty Zarzadu KOT',
            'documents' => $documents,
            'selected' => $selected,
            'content' => $content,
            'can_edit' => true
