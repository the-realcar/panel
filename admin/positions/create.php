...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        redirect('/admin/positions/create.php');
    }
    ...
    if (empty($errors['name'])) {
        $check_query = "SELECT COUNT(*) as count FROM positions WHERE name = :name";
        $check_result = $db->queryOne($check_query, [':name' => $form_data['name']]);
    ...
    if (empty($errors)) {
        try {
            ...
            setFlashMessage('success', 'Stanowisko zostało dodane pomyślnie.');
            redirect('/admin/positions/index.php');
        } catch (Exception $e) {
            error_log('Error creating position: ' . $e->getMessage());
            $errors['general'] = 'Wystąpił błąd podczas dodawania stanowiska.';
        }
    }
}
...
<div class="card">
    <div class="card-body">
        ...
        <form method="POST" action="/admin/positions/create.php">
            <?php echo csrfField(); ?>
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-primary">💾 Zapisz stanowisko</button>
