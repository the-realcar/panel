...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        redirect('/admin/vehicles/create.php');
    }
    ...
    if (empty($errors['vehicle_number'])) {
        $check_query = "SELECT COUNT(*) as count FROM vehicles WHERE vehicle_number = :vehicle_number";
        $check_result = $db->queryOne($check_query, [':vehicle_number' => $form_data['vehicle_number']]);
    ...
    // Check if registration_plate is unique (if provided)
    if (!empty($form_data['registration_plate']) && empty($errors['registration_plate'])) {
        $check_query = "SELECT COUNT(*) as count FROM vehicles WHERE registration_plate = :registration_plate";
        $check_result = $db->queryOne($check_query, [':registration_plate' => $form_data['registration_plate']]);
    ...
    if (empty($errors)) {
        try {
            ...
            setFlashMessage('success', 'Pojazd został dodany pomyślnie.');
            redirect('/admin/vehicles/index.php');
        } catch (Exception $e) {
            error_log('Error creating vehicle: ' . $e->getMessage());
            $errors['general'] = 'Wystąpił błąd podczas dodawania pojazdu.';
        }
    }
}
